...
#' @export
#'
get_complete_scatter_dt <- function(se, ain=NULL){
  complete_scatter_dt <- NULL
  if(is.null(ain)){
    assays <- names(SummarizedExperiment::assays(se))
  } else {
    assays <- ain
  ...
  for(ain in assays){
    if (ain != "raw"){
      dt <- SummarizedExperiment::assays(se)[[ain]]
      fit <- stats::lm(scatter_dt$A ~ scatter_dt$B)
      scatter_dt <- data.table::data.table("A" = dt[[1]], "B" = dt[[2]], Normalization = paste0(ain, " (R2 = ", format(summary(fit)$adj.r.squared, digits=2), ")"))
      if (is.null(complete_scatter_dt)){
        complete_scatter_dt <- scatter_dt
      } else {
        complete_scatter_dt <- rbind(complete_scatter_dt, scatter_dt)
      }
    }
  }
  return(complete_scatter_dt)
...
plot_densities <- function(se, ain = NULL, color_by = NULL, facet_norm = TRUE, ncol = 3){
  ...
  if(facet_norm){
    p <- ggplot2::ggplot(melted_dt, ggplot2::aes(x=get("Intensity"), color=get(color_by))) +
      ...
      ggplot2::facet_wrap(~Assay, scales="free", ncol = 4) +
...
plot_intragroup_PCV <- function(se, ain = NULL, condition = NULL, diff = FALSE){
  ...
  # prepare data
  avgCVPerNormAndReplicates <- calculateReplicateCV(assays, data.table::as.data.table(SummarizedExperiment::colData(se))[,condition])
