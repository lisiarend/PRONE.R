...
#' @param value String specifying the value for the removal of samples (samples with this value in the specified column will be removed)
#'
#' @return filtered SummarizedExperiment object
#' @export
#'
remove_samples_manually <- function(se, column, value){
  ...
  if(! column %in% colnames(coldata)){
    ...
    if(! value %in% col){
      stop(paste0("No sample found with ", value, " in ", column, "column!"))
    } else {
      se_subset <- se[, se[[column]] != value]
...
detect_outliers_POMA <- function(se, ain="raw", condition = NULL, method="euclidean", type="median", group=TRUE, coeff = 1.5){
  ...
  poma_res <- POMA::PomaOutliers(data, do="analyze", method=method, type=type, coef = coeff)
  ...
  poma_res$polygon_plot <- poma_res$polygon_plot +
    ...
    ggplot2::scale_shape_manual(name = condition, values = 1:length(unique(md[, condition])))
